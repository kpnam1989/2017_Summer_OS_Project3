...
int main(int argc, char **argv) {
  ...
  struct WorkerFuncData thisData[nworkerthreads];
  for(i = 0; i < nworkerthreads; i++) {
	 thisData[i].thisServer = server;
	 thisData[i].uniqueID = i;
     gfserver_setopt(&gfs, GFS_WORKER_ARG, i, &thisData[i]);
  }
